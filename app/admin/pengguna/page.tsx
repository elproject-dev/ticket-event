    ...
    <div className="p-4 pb-24">
      <div className="container mx-auto">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-1x1 font-bold tracking-widest text-primary uppercase">Kelola Pengguna</h1>
        </div>
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-muted/50 uppercase tracking-widest border-b">
                <tr>
                  ...
                  <th className="p-3 font-bold text-right">Tanggal Bergabung</th>
                ...
                {users.length > 0 ? users.map((user) => (
                  <tr key={user.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="p-3 text-right text-muted-foreground">
                      {new Date(user.dibuat_pada).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </td>
                    <td className="p-3 text-center">
                      <UserActionMenu userId={user.id} peran={user.peran} />
                    </td>
                  </tr>
                )) : (
                  <tr>
                    <td colSpan={6} className="p-4 text-center text-muted-foreground border-dashed border">Belum ada pengguna</td>
                  </tr>
                )}
              </tbody>
            </table>
