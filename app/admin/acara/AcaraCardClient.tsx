...
export default function AcaraCardClient({ acara }: { acara: acara }) {
  ...
  const handleEdit = (e: React.MouseEvent) => {
    e.stopPropagation();
    router.push(`/admin/acara/edit/${acara.id}`);
  };
      ...
      >
        <div className="w-full aspect-video bg-muted relative shrink-0">
          {acara.url_gambar ? (
            ...
            <img src={acara.url_gambar} alt={acara.judul} className="object-cover w-full h-full absolute inset-0" />
          ) : (
        ...
        <CardContent className="p-4 flex-1 space-y-1">
          <h3 className="font-semibold text-sm line-clamp-1">{acara.judul}</h3>
          <div className="flex justify-between items-start mt-1">
            <div className="flex flex-col text-[10px] text-muted-foreground gap-1 flex-1">
              <div>
                {formatTanggalIndo(acara.tanggal_mulai)} - {formatTanggalIndo(acara.tanggal_selesai)}
              </div>
            ...
            <div className="flex flex-col items-end shrink-0 pl-2 gap-0.5">
              <span className="text-[10px] text-muted-foreground font-medium">{formatJam(acara.tanggal_mulai)} - {formatJam(acara.tanggal_selesai)} WIB</span>
              <span className="text-xs font-bold text-primary">Rp {Number(acara.harga).toLocaleString("id-ID")}</span>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="space-y-5">
            {acara.url_gambar && (
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    acara.url_gambar, 
                    acara.url_sub_gambar_1, 
                    acara.url_sub_gambar_2, 
                    acara.url_sub_gambar_3, 
                    // @ts-ignore
                    acara.url_sub_gambar_4, 
                    // @ts-ignore
                    acara.url_sub_gambar_5
                  ].map((gambar, idx) => (
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between items-center border-b pb-2 mb-2">
                ...
                <span className="font-medium text-xs">{formatTanggalIndo(acara.tanggal_mulai)} - {formatTanggalIndo(acara.tanggal_selesai)}</span>
              </div>

              <div className="flex justify-between items-center border-b pb-2 mb-2">
                <span className="font-semibold text-[10px] uppercase tracking-wider text-muted-foreground block">Waktu</span>
                <span className="font-medium text-xs">{formatJam(acara.tanggal_mulai)} - {formatJam(acara.tanggal_selesai)} WIB</span>
              </div>

              <div className="flex justify-between items-center border-b pb-2 mb-2">
                <span className="font-semibold text-[10px] uppercase tracking-wider text-muted-foreground block">Lokasi</span>
                <span className="font-medium text-xs text-right max-w-[200px] truncate">{acara.lokasi}</span>
              </div>

              {acara.url_maps && (
                <div className="flex justify-between items-center border-b pb-2 mb-2">
                  <span className="font-semibold text-[10px] uppercase tracking-wider text-muted-foreground block">Peta Lokasi</span>
                  <a href={acara.url_maps} target="_blank" rel="noreferrer" className="text-primary text-xs underline underline-offset-2 text-right">
                    Buka di Google Maps
                  </a>
                </div>
              )}
              ...
              <div className="flex justify-between items-center border-b pb-2 mb-2">
                ...
                <span className="font-medium text-xs">{acara.nama_penyelenggara || "MANAJEMENTIKET"}</span>
              </div>
              ...
              <div className="flex justify-between items-center border-t border-b py-2 my-2">
                <div>
                  ...
                  <span className="font-bold text-primary text-base">Rp {Number(acara.harga).toLocaleString("id-ID")}</span>
                </div>
                {acara.diskon && Number(acara.diskon) > 0 ? (
                  <div className="text-right">
                    <span className="font-semibold text-[10px] uppercase tracking-wider text-muted-foreground block mb-0.5">Diskon</span>
                    <span className="text-emerald-600 font-bold text-base">Rp {Number(acara.diskon).toLocaleString("id-ID")}</span>
                  </div>
                ) : null}
              </div>
              ...
              <div className="flex justify-between items-center border-b pb-2 mb-2">
                ...
                <span className="font-medium text-xs">{acara.kapasitas.toLocaleString("id-ID")} Orang</span>
              </div>

              <div className="flex justify-between items-center border-b pb-2 mb-2">
                <span className="font-semibold text-[10px] uppercase tracking-wider text-muted-foreground block shrink-0">Deskripsi Acara</span>
                <span className="font-medium text-xs text-right truncate ml-4 max-w-[200px] text-muted-foreground">{acara.deskripsi}</span>
              </div>
            </div>
          </div>
        </DialogContent>
